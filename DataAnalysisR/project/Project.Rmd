...
loanDataFull <- read.csv("/Users/abhiso/Work/learn/Udacity/DataAnalysisR/project/prosperLoanData.csv", sep = ",")
...
columns <- c("Term", "LoanStatus", "ClosedDate", "ListingCategory..numeric.", "BorrowerState", "Occupation", "IncomeRange", "IncomeVerifiable", "StatedMonthlyIncome", "CreditScoreRangeLower", "ProsperScore", "EmploymentStatus", "EmploymentStatusDuration", "CurrentCreditLines", "TotalCreditLinespast7years", "DebtToIncomeRatio", "BorrowerRate", "LoanOriginalAmount", "LoanOriginationDate")
...
```{r, echo=FALSE, Term}
qplot(Term, data = loanData, binwidth=1)
table(loanData$Term)
```
...
qplot(LoanStatus, data = loanData)
...
qplot(data = loanData, Occupation)
...
```{r, echo=FALSE, subsetsOfOccupation}
qplot(data = subset(loanData, !(loanData$Occupation %in% c("Other", "Professional"))), Occupation) + coord_flip()
```

There are maximum Computer Programmers

```{r, echo=FALSE, CreditScoreRatings}
qplot(data = loanData, CreditScoreRangeLower)
```

There are few outliers with 0 credit score, need to remove those

```{r, echo=FALSE, RemovingOutliersCreditScoreRatings}
qplot(data = subset(loanData, loanData$CreditScoreRangeLower > 450), CreditScoreRangeLower, binwidth = 50)
...
qplot(data = subset(loanData, loanData$StatedMonthlyIncome < 10000), StatedMonthlyIncome)
```
...
qplot(data = subset(loanData, loanData$StatedMonthlyIncome < 10000), StatedMonthlyIncome, binwidth = 1000)
```
...
qplot(data = subset(loanData, !is.na(ClosedDateYear)), ClosedDateYear, binwidth = 1)
```
...
qplot(data = subset(loanData, CurrentCreditLines < 30), CurrentCreditLines, binwidth = 0.5)
...
qplot(data = subset(loanData, DebtToIncomeRatio <= 1.0), DebtToIncomeRatio, binwidth=0.01)
```
...
qplot(data = subset(loanData, StatedMonthlyIncome < 12000 & StatedMonthlyIncome > 0 & CreditScoreRangeLower > 450), x = CreditScoreRangeLower, y = StatedMonthlyIncome, alpha = I(1/50)) + geom_smooth()
```
...
qplot(data = subset(loanData, StatedMonthlyIncome < 12000 & StatedMonthlyIncome > 0 & CreditScoreRangeLower > 450), x = CreditScoreRangeLower, y = StatedMonthlyIncome, alpha = I(1/50))
```
...
qplot(data = subset(loanData, StatedMonthlyIncome < 12000 & StatedMonthlyIncome > 0 & CreditScoreRangeLower > 450), x = CreditScoreRangeLower, y = StatedMonthlyIncome, alpha = I(1/50)) + geom_smooth()
loanDataIncome <- subset(loanData, StatedMonthlyIncome < 12000 & StatedMonthlyIncome > 0 & CreditScoreRangeLower > 450)
corIncomeVsScore <- cor(loanDataIncome$StatedMonthlyIncome, loanDataIncome$CreditScoreRangeLower)
```
...
qplot(data = subset(loanData, !is.na(ProsperScore)), x = ProsperScore, y = CreditScoreRangeLower, alpha = I (1/20)) + geom_smooth()
loanDataProsperCredit <- subset(loanData, !is.na(ProsperScore) & CreditScoreRangeLower > 450)
corProsperVsCredit <- cor(loanDataProsperCredit$CreditScoreRangeLower, loanDataProsperCredit$ProsperScore)
```
...
qplot(CreditScoreRangeLower, data = subset(loanData, CreditScoreRangeLower > 450), binwidth = 20, fill = LoanStatus) +
...
qplot(CreditScoreRangeLower, data = subset(loanData, CreditScoreRangeLower > 450 & !(loanData$LoanStatus %in% c("Current")) & !is.na(ClosedDateYear)), binwidth = 20, fill = LoanStatus) +
  guides(fill = guide_legend(reverse = T))
```
...
qplot(ClosedDateYear, data = subset(loanData, !is.na(ClosedDateYear)), fill = LoanStatus, binwidth = 1) + guides(fill = guide_legend(reverse = T))
```
...
```{r, echo=FALSE, LoanStatuVsState}
qplot(BorrowerState, data = subset(loanData, !is.na(ClosedDateYear)), fill = LoanStatus) + guides(fill = guide_legend(reverse = T)) + coord_flip()
```
...
qplot(ListingCategory..numeric., data = subset(loanData, !is.na(ClosedDateYear)), fill = LoanStatus, binwidth = 1) + guides(fill = guide_legend(reverse = T))
```
...
qplot(y = CurrentCreditLines, x = CreditScoreRangeLower, data = subset(loanData, CreditScoreRangeLower > 450 & !is.na(CurrentCreditLines)), alpha = I(1/400)) + geom_smooth()
by(loanData$CurrentCreditLines, loanData$CreditScoreRangeLower, summary)
```
...
qplot(DebtToIncomeRatio, data = subset(loanData, DebtToIncomeRatio <= 1.0 & !(loanData$LoanStatus %in% c("Current")) & !is.na(ClosedDateYear)), fill = LoanStatus, binwidth = 0.01) + guides(fill = guide_legend(reverse = T))

qplot(DebtToIncomeRatio, data = subset(loanData, DebtToIncomeRatio <= 1.0 & !(loanData$LoanStatus %in% c("Current")) & !is.na(ClosedDateYear)), fill = LoanStatus, binwidth = 0.01) + facet_wrap(~LoanStatus)
```
...
qplot(EmploymentStatus, data = subset(loanData, !is.na(ClosedDateYear)), fill = LoanStatus) + guides(fill = guide_legend(reverse = T)) + coord_flip() + facet_wrap(~LoanStatus)
```
...
qplot(LoanYear, data = subset(loanData, !is.na(ClosedDateYear) & LoanYear > 2005), fill = LoanStatus, binwidth=1) + guides(fill = guide_legend(reverse = T))
```
...
loanData$MonthlyDebt <- loanData$StatedMonthlyIncome * loanData$DebtToIncomeRatio
...
qplot(x = CurrentCreditLines, y = MonthlyDebt, data = subset(loanData, MonthlyDebt < 8000 & CurrentCreditLines < 40), alpha = I(1/20))

loanDataSub <- subset(loanData, MonthlyDebt < 8000 & CurrentCreditLines < 40)
corMonthlyDebtToCreditLines <- cor(loanDataSub$CurrentCreditLines, loanDataSub$MonthlyDebt)
```
...
qplot(x = CreditScoreRangeLower, y = BorrowerRate, data = subset(loanData, CreditScoreRangeLower > 450), alpha = I(1/50)) + geom_smooth()
loanDataScore <- subset(loanData, CreditScoreRangeLower > 450)
corCreditScoreToBorroweRate <- cor(loanDataScore$CreditScoreRangeLower, loanDataScore$BorrowerRate)
```
...
qplot(y = BorrowerRate, x = LoanOriginalAmount, data = loanData, alpha = I(1/50))
...
qplot(y = LoanOriginalAmount, x = CreditScoreRangeLower, data = loanDataScore, alpha = I(1/100))
corAmountVsScore = cor(loanDataScore$CreditScoreRangeLower, loanDataScore$LoanOriginalAmount)
```
...
qplot(BorrowerRate, data = subset(loanData, BorrowerRate < 0.36), fill = LoanStatus, binwidth = 0.005) + guides(fill = guide_legend(reverse = T))
```
...
ggplot(aes(x = LoanOriginalAmount, y = BorrowerRate), data = subset(loanDataScore, CreditScoreRangeLower > 680)) + 
  geom_point(aes(color = factor(CreditScoreRangeLower)), alpha = 0.2, size = 1, position = 'jitter') +
...
```{r, echo=FALSE, MonthlyDebtCreditLineCreditScore}
ggplot(data = subset(loanData, !(loanData$IncomeRange %in% c("$0"))), aes(LoanYear, LoanOriginalAmount)) +
...
ggplot(aes(x = ProsperScore, y = DebtToIncomeRatio), data = subset(loanDataScore, !is.na(ProsperScore) & CreditScoreRangeLower < 820 & DebtToIncomeRatio < 0.75)) + 
  geom_point(aes(color = factor(CreditScoreRangeLower)), alpha = 0.5, size = 1, position = 'jitter') +
  scale_color_brewer(type = 'div',
                     guide = guide_legend(title = 'Credit Score', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
...
ggplot(aes(x = CurrentCreditLines, y = MonthlyDebt), data = subset(loanData, MonthlyDebt < 8000 & CurrentCreditLines < 40 & CreditScoreRangeLower < 820 & CreditScoreRangeLower > 600)) + 
  geom_point(aes(color = factor(CreditScoreRangeLower)), alpha = 0.5, size = 1, position = 'jitter') +
  scale_color_brewer(type = 'div',
                     guide = guide_legend(title = 'Credit Score', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
...
### Plot One
```{r, echo=FALSE, YearVsLoanStatus1}
...
```{r, echo=FALSE, AmountVsIncomeRange1}
ggplot(data = subset(loanData, DebtToIncomeRatio < 1.0), aes(DebtToIncomeRatio, LoanOriginalAmount)) +
  geom_point(aes(color = factor(IncomeRange)), alpha = 0.5, size = 1, position = 'jitter') +
  facet_wrap(~LoanYear) +
  scale_color_brewer(type = 'div',
                     guide = guide_legend(title = 'Income Range', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
```
...
### Plot Three
```{r, echo=FALSE, DebtToIncomeMulti1}
ggplot(aes(x = ProsperScore, y = DebtToIncomeRatio), data = subset(loanDataScore, !is.na(ProsperScore) & CreditScoreRangeLower < 820 & DebtToIncomeRatio < 1.0)) + 
  geom_point(aes(color = factor(CreditScoreRangeLower)), alpha = 0.1, size = 1, position = 'jitter') +
  facet_wrap(~IncomeRange) +
  scale_color_brewer(type = 'div',
                     guide = guide_legend(title = 'Credit Score', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
